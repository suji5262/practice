...
// 스프링의 @Controller + @RequestMappping 과 같음
@WebServlet("/myDispatcherServlet")  // http://localhost/ch2/myDispatcherServlet?year=2021&month=10&day=1
public class MyDispatcherServlet extends HttpServlet { // 꼭 HttpServlet 상속을 받아야함
	// 서블릿에서는 메서드 단위로 맵핑 x > 클래스 단위로 맵핑 ok , 스프링에 비해 클래스를 많이 만들어야함
	@Override
	// 메서드는 꼭 service 로 받아야하며
	// HttpServletRequest request, HttpServletResponse response > 매개변수를 받아야함
	// 위에 두개는 고정!
